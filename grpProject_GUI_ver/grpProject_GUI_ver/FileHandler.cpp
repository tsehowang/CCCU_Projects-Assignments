...
void FileHandler::ReadUser(std::string fileLocation) {
	txtInput.open(fileLocation);
	if (txtInput.is_open())
	{
...
void FileHandler::ReadEquipment(std::string fileLocation) {
	txtInput.open(fileLocation);
	if (txtInput.is_open())
	{
		while (getline(txtInput, line)) {
...
void FileHandler::writeCurrentStatus(std::string fileLocation) {
	std::string temp = "";
	txtInput.open(fileLocation);
	if (txtInput.is_open())
	{
...
void FileHandler::clearCurrentStatus() {
	txtOutput.open("currentStatus.txt",std::ofstream::out | std::ofstream::trunc);
	txtOutput.close();

}

void FileHandler::clearCampEquipment() {
	txtOutput.open("camp_equipment.txt", std::ofstream::out | std::ofstream::trunc);
	txtOutput.close();
}

void FileHandler::writeUser() {
	std::string outputStorer;
	if (!allUsers.empty()) {
		outputStorer += allUsers.at(0)->getUserInfo();
		for (int i = 1; i < allUsers.size(); i++) {
			outputStorer += "\n" + allUsers.at(i)->getUserInfo();
		}
	}
	txtOutput.open("user.txt");
	txtOutput << outputStorer;
	txtOutput.close();
}
// for admin
